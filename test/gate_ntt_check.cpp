          ...
          const SecretKey& sk, const EvalKey& ek)
{
    ...
    for (int i = 0; i < kNumTests; i++) {
        if constexpr (std::is_invocable_v<Func, TLWE<P>&>) {
            func(cres[i]);
            p[i] = chegk();
            cout << "Output " << p[i] << endl;
        }
        else if constexpr (std::is_invocable_v<Func, TLWE<P>&,
                const TLWE<P>&>) {
            func(cres[i], c[i]);
            cout << "Input " << static_cast<int>(p[i]) << endl;
            p[i] = chegk(p[i]);
            cout << "Output " << static_cast<int>(p[i]) << endl;
        }
        else if constexpr (std::is_invocable_v<Func, TLWE<P>&, const TLWE<P>&,
                const TLWE<P>&,
                const EvalKey&>) {
            func(cres[i], c[i], c[i + kNumTests], ek);
            cout << "Input " << static_cast<int>(p[i]) << " " << static_cast<int>(p[i + kNumTests]) << endl;
            ...
            cout << "Input " << static_cast<int>(p[i]) << " " << static_cast<int>(p[i + kNumTests]) << " " << static_cast<int>(p[i + kNumTests * 2]) << endl;
            ...
            func(cres[i], c[i], c[i + kNumTests], c[i + kNumTests * 2],
                 c[i + kNumTests * 3], ek);
            cout << "Input " << static_cast<int>(p[i]) << " " << static_cast<int>(p[i + kNumTests]) << " " << static_cast<int>(p[i + kNumTests * 2]) << " " << static_cast<int>(p[i + kNumTests * 3]) << endl;
            p[i] = chegk(p[i], p[i + kNumTests], p[i + kNumTests * 2],
                         p[i + kNumTests * 3]);
            cout << "Output " << static_cast<int>(p[i]) << endl;
        }
