...
{
    TFHEpp::HomXOR_NTT<iksP, brP, brP::targetP::mu>(sum, ca, cb, ek);
    TFHEpp::HomAND_NTT<iksP, brP, brP::targetP::mu>(carry, ca, cb, ek);
}
               ...
               const TFHEpp::EvalKey& ek)
{
    ...
    TFHEpp::HomOR_NTT<iksP, brP, brP::targetP::mu>(carry, hcarryabc, hcarryab, ek);
}
...
int main()
{
    ...
    for (int i = 0; i < 16; i++) {
        ...
        FullAdder(tmpsum, flagclient, clientcipher[i], cloudcipher[i],
                  flagclient, ek);
        TFHEpp::HomANDYN_NTT<iksP, brP, brP::targetP::mu>(flagequiv, flagequiv,
                                                     tmpsum, ek);
    }
